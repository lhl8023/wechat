...
* 2. void I2C_SendByte(uint8_t addr,uint8_t data) -- 向寄存器地址写一个byte的数据
...
#define IIC_DATA_H()        GPIO_SetBits(IIC_DATA_PORT, IIC_DATA_PIN)                  //拉高DATA数据线
#define IIC_DATA_L()        GPIO_ResetBits(IIC_DATA_PORT, IIC_DATA_PIN)                //拉低DATA数据线
#define IIC_DATA_R()        GPIO_ReadInputDataBit(IIC_DATA_PORT, IIC_DATA_PIN)         //读DATA数据线

#define IIC_SCK_H()        	GPIO_SetBits(IIC_SCK_PORT, IIC_SCK_PIN)                    //拉高SCK时钟线
#define IIC_SCK_L()        	GPIO_ResetBits(IIC_SCK_PORT, IIC_SCK_PIN)                  //拉低SCK时钟线
#define IIC_SCK_R()        	GPIO_ReadInputDataBit(IIC_SCK_PORT, IIC_SCK_PIN)         	//读SCK数据线
	...
	switch(TextSize)
	{
		...
		case 4:
		{
			while(ch[j] != '\0')
			{
				...
				OLED_SetPos(x,y+2);
				for(i=0;i<8;i++)
					WriteDat(F8X16[c*16+i+16]);
				OLED_SetPos(x,y+3);
				for(i=0;i<8;i++)
					WriteDat(F8X16[c*16+i+24]);
				x += 8;
				j++;
			}
		}break;
