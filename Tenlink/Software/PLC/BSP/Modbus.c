...
unsigned char Data_ModBus_S[50], PLC_Data_ModBus_M[50], VOC_Data_ModBus_M[15];
unsigned char uchCRCHi; //* 高CRC字节初始化 *//
unsigned char uchCRCLo; //* 低CRC 字节初始化 *//
unsigned int  code_crc;
unsigned char flag_echo_master=0, flag_send_master=0, PLC_flag_echo_master=0, VOC_flag_echo_master=0;
unsigned char flag_DeviceNoEcho=0, Start_DeviceNoEcho=0;
unsigned int  num_DeviceNoEcho=0;
unsigned char PLC_flag_DeviceNoEcho=0, PLC_Start_DeviceNoEcho=0;
unsigned int  PLC_num_DeviceNoEcho=0;
...
void T15EndHandle_Slaver(void)
{
  ...
  if(modbusdata_slaver.recv[0] == Header_Zigbee_Send)          //1、接收数据包头判定
  {      
    if(modbusdata_slaver.recv[modbusdata_slaver.length_recv-1] == XOR(modbusdata_slaver.recv,modbusdata_slaver.length_recv-1) )  //2、异或和校验位判定
    {
      if(modbusdata_slaver.length_recv == modbusdata_slaver.recv[4]+6)       //3、接收数据长度判定,modbusdata_slaver.recv[4]为负载长度
      {
        modbusdata_slaver.FrameOK = 1;
        modbusdata_slaver.length_buf = modbusdata_slaver.recv[4];//modbusdata_slaver.buf为上位机发送的控制指令（去除Zigbee相关帧） 
        memcpy(modbusdata_slaver.buf, &modbusdata_slaver.recv[5], modbusdata_slaver.length_buf);      
      }
    }
  }
    memset(modbusdata_slaver.recv, 0, 50);
...
void MBus_Dispose_Slaver(void)
{
    ...
    if(modbusdata_slaver.buf[0]==ADDR_MODBUS_M)
      Protocol_Zigbee(modbusdata_slaver.buf,sizeof(modbusdata_slaver.buf)); //Data_ModBus_S为接收到的待处理数据，send为生成的缓存数据

    memset(modbusdata_slaver.buf, 0, 50);
    return;
}
...
void T15EndHandle_master(void)
{
    ...
    {    
       if(PLC_modbusdata_master.recv[1] == FUNCTION03_MASTER)
       { 

            PLC_modbusdata_master.FrameOK = 1;
            PLC_modbusdata_master.length_buf = PLC_modbusdata_master.length_recv;
            memcpy(PLC_modbusdata_master.buf, PLC_modbusdata_master.recv, PLC_modbusdata_master.length_buf);         
        
       }
//       else if(PLC_modbusdata_master.recv[1] == FUNCTION06_MASTER)
//       { 
//        if(PLC_modbusdata_master.length_recv <= NUM_DATA_RECV_S)
//        {
//            PLC_modbusdata_master.FrameOK = 1;
//            PLC_modbusdata_master.length_buf = PLC_modbusdata_master.length_recv;
//            memcpy(PLC_modbusdata_master.buf, PLC_modbusdata_master.recv, PLC_modbusdata_master.length_buf);         
...
//       }
    ...
    memset(PLC_modbusdata_master.recv, 0, 50);
...
void PLC_MBus_Dispose_master(void)
{
    ...
    if(CRC16(PLC_modbusdata_master.buf,PLC_modbusdata_master.length_buf-2, 0) == crc16_decode)    //CRC16校验正确
    {      
        Protocol_PLC(PLC_modbusdata_master.buf,PLC_modbusdata_master.length_buf);
    }

    memset(PLC_modbusdata_master.buf, 0, 50);
    return;
