...
unsigned char ReceiveData[50], SendData_Uart1[20], SendData_Uart2[20],SendData_Uart3[20], SendData_Uart4[20], num_ReceiveData=0, flag_send_uart1=0, flag_send_uart2=0, flag_send_uart3=0, flag_send_uart4=0;
unsigned char Send_Num_data1=0, SendNum_Over1=0, Send_Num_data2=0, SendNum_Over2=0, Send_Num_data3=0, SendNum_Over3=0, Send_Num_data4=0, SendNum_Over4=0;
unsigned char flag_485_SendDelay=0, Longth_Uart1Send=0, Longth_Uart2Send=0, Longth_Uart3Send=0, Longth_Uart4Send=0;
...
//  通过UART3发送字符串
void Uart3Putstring(const char *s)
{    
    while (*s != '\0')
    {
...
void Uart3_SendStart(unsigned char* data_send, unsigned char num)
{
    Send_Num_data3 = 0;
    SendNum_Over3 = num;
    //清发送完成中断标志位
    ...
    USART3->DR = data_send[0];
    Send_Num_data3++;
}

void Uart3_SendStop(void)
{
    //关闭发送完成中断
...
//参数：收到的字符
//返回值：无
void ReceOneChar_Uart3(unsigned char ReceCharacter)
{
    ReceiveData[num_ReceiveData++] = ReceCharacter;
    if(num_ReceiveData >= 20)
    {
...
void SZ_STM32_Uart4Init(unsigned int BaudRate)
{
    GPIO_InitTypeDef GPIO_InitStructure;  
    USART_InitTypeDef USART_InitStructure; 
...
//  通过UART4发送字符串
void Uart4Putstring(const char *s)
{    
    while (*s != '\0')
    {
...
void Uart4_SendStart(unsigned char* data_send, unsigned char num)
{
    Send_Num_data4 = 0;
    SendNum_Over4 = num;
    //清发送完成中断标志位
    ...
    UART4->DR = data_send[0];
    Send_Num_data4++;
}

void Uart4_SendStop(void)
{
    //关闭发送完成中断
...
//参数：收到的字符
//返回值：无
void ReceOneChar_Uart4(unsigned char ReceCharacter)
{
    ReceiveData[num_ReceiveData++] = ReceCharacter;
    if(num_ReceiveData >= 20)
    {
...
void NVIC_Uart4Configuration(void)
{
    NVIC_InitTypeDef NVIC_InitStructure;  
