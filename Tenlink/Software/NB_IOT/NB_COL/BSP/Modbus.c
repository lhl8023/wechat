...
void T15EndHandle_Slaver(void)
{
 ...
 u8 csq_data[5];//临时缓存信号质量
  ...
  {      
    ...
    else if(modbusdata_slaver.recv[0]== 0x2B&&modbusdata_slaver.recv[1]== 0x43&&modbusdata_slaver.recv[2]== 0x53&&modbusdata_slaver.recv[3]== 0x51&&modbusdata_slaver.recv[4]== 0x3a)  
      {
      
         csq_data[0]=HexToChar(modbusdata_slaver.recv[6]);
         csq_data[1]=HexToChar(modbusdata_slaver.recv[7]);
     
   
             CSQ[0]=(csq_data[0]<<4)|csq_data[1];
